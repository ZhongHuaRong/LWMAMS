...
ChartView {
    ...
    function setYearChart(data,year,dataType){
        ...
        view.title = year.split(' ')[0].split('/')[0]+"年 "+dataType+"情况"
    }
    ...
    PieSeries {
        ...
        CPieSlice {
            ...
            label: {
                if(isValue)
                    return "异常值(大于最大值):"+ moreThan.value;
                else
                {
                    if(sumCount ==0)
                        return "异常值(大于最大值): 0%"
                    else
                        return "异常值(大于最大值): "+(moreThan.value/sumCount*100).toFixed(2)+"%"
                }
            }
            value: moreCount
            color:"#9602FC"
        ...
        CPieSlice {
            ...
            label: {
                if(isValue)
                    return "异常值(小于最小值):"+ lessThan.value;
                else
                {
                    if(sumCount ==0)
                        return "异常值(小于最小值): 0%"
                    else
                        return "异常值(小于最小值): "+(lessThan.value/sumCount*100).toFixed(2)+"%"
                }
            }
            value: lessCount
            color:"#FCB407"
        ...
        CPieSlice {
            ...
            label: {
                if(isValue)
                    return "正常值:"+ normal.value;
                else
                {
                    if(sumCount ==0)
                        return "正常值: 0%"
                    else
                        return "正常值: "+(normal.value/sumCount*100).toFixed(2)+"%"
                }
            }
            value: norCount
            color:"#03B3D4"
    ...
    Text{
        ...
        anchors.leftMargin: view.width / 2.8
