...
{
    QStringList list;
    list<<QString::number(TcpClient::PC)<<name<<pw;
...
void ClientManagement::checkNewData()
{
    ...
    bool isPushTemp = false;
    bool isPushPH = false;
    bool isPushTur = false;

    if(tem<para->getTempMinValue().toDouble()||tem>para->getTempMaxValue().toDouble())
    {
        isPushTemp = true;
    }

    if(ph<para->getPHMinValue().toDouble()||ph>para->getPHMaxValue().toDouble())
    {
        isPushPH = true;
    }

    if(tur<para->getTurMinValue().toDouble()||tur>para->getTurMaxValue().toDouble())
    {
        isPushTur = true;
    }

    pushSMSAndEmail(isPushTemp,isPushPH,isPushTur,
                    para->getNSMSPush(),para->getNEmailPush(),
                    para->getSPhone(),para->getSEmail());
}
